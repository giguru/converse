...
class Converse:
    ...
    def __init__(self,
        ...
        """
        Initialize a Converse instance.
        :param reader: Reader instance
        :param retrievers:
        """
        if len(retrievers) is 0:
            raise AttributeError("Please provide at least one retriever")
    ...
    def get_answers(self, question: str, top_k_reader: int = 1, top_k_retriever: int = 10,
        ...
        results = self.__reader.predict(question=question, documents=documents, top_k=top_k_reader)  # type: PredictionResult
    ...
    def __go_through_retrievers(self, questions: List[str], filters: Optional[dict], top_k_retriever: int):
        ...
        for idx, retriever in enumerate(self.__retrievers):
            if idx == 0:
                ...
                documents = retriever.follow_up_retrieve(questions, documents, filters=filters, top_k=top_k_retriever)
    ...
    def eval(self, label_index: str, label_origin: str = "gold_label", top_k_retriever: int = 10, top_k_reader: int = 10):
        """
    ...
    def calc_eval_results(eval_counts: Dict):
        ...
        eval_results["reader_top1_accuracy_has_answer"] = eval_counts[
                                                              "correct_readings_top1_has_answer"] / number_of_has_answer
        eval_results["reader_topk_accuracy"] = eval_counts["correct_readings_topk"] / correct_retrievals
        eval_results["reader_topk_accuracy_has_answer"] = eval_counts[
                                                              "correct_readings_topk_has_answer"] / number_of_has_answer
        ...
        if number_of_no_answer:
            eval_results["reader_top1_no_answer_accuracy"] = eval_counts[
                                                                 "correct_no_answers_top1"] / number_of_no_answer
            eval_results["reader_topk_no_answer_accuracy"] = eval_counts[
                                                                 "correct_no_answers_topk"] / number_of_no_answer
        else:
